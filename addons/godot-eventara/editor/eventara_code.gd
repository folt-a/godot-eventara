...
#-----------------------------------------------------------
func _ready():
	if interface.get_script_editor() != null:
		if interface.get_script_editor().get_current_editor() != null:
			var base_editor = interface.get_script_editor().get_current_editor().get_base_editor() as CodeEdit
			syntax_highlighter = base_editor.syntax_highlighter

#	syntax_highlighter = preload("res://addons/godot-eventara/editor/syntax_highlighter.gd").new()
#	syntax_highlighter.init(interface.get_editor_settings())
	...
	text_changed.connect(_on_text_editor_text_changed)
...
#		add_code_completion_option(CodeEdit.KIND_PLAIN_TEXT, 'Join', 'Join ', syntax_highlighter.function_color, load('res://addons/dialogic/Editor/Images/Dropdown/join.svg'))
#		add_code_completion_option(CodeEdit.KIND_PLAIN_TEXT, 'Leave', 'Leave ', syntax_highlighter.function_color, load('res://addons/dialogic/Editor/Images/Dropdown/leave.svg'))
#		add_code_completion_option(CodeEdit.KIND_PLAIN_TEXT, 'Update', 'Update ', syntax_highlighter.function_color, load('res://addons/dialogic/Editor/Images/Dropdown/update.svg'))
		pass
...
#15. public methods
#-----------------------------------------------------------
func load_(eventara:Eventara):
	text = ""

	var result:String = ""
	var indent := 0
	for cmd in eventara.commands:
		if cmd['event_name'] == 'End Branch':
			indent -= 1
			continue
...
#				result += "\t".repeat(indent)+"\n"
			result += "\t".repeat(indent)+cmd['event_node_as_text'].replace('\n', "\n"+"\t".repeat(indent)) + "\n"



		if cmd.can_contain_events:
			indent += 1
		if indent < 0:
			indent = 0
